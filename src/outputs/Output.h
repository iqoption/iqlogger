...
namespace iqlogger::outputs {
    ...
    public:
        ...
        explicit Output(config::DestinationConfig destinationConfig) :
            ...
            m_send_counter (0)
        {
            DEBUG("Initialize " << m_name << " Initial counter " << m_send_counter.load() << " IsLockFree: " << m_send_counter.is_lock_free());
        }

        virtual bool importMessages(message_ptr_buffer_t::const_iterator it, size_t count) const override
        {
            ...
            for(size_t i = 0; i < count; ++i)
            {
                ...
                std::visit([&recordPtr](const auto& mess) {
        ...
        virtual void start() override
        {
            ...
            metrics::MetricsController::getInstance()->registerMetric("outputs." + m_name + ".send_total", [this]() {
