...
namespace iqlogger::config {
    ...
    public:
        ...
        static constexpr frozen::string protocol_to_str(Protocol p)
        {
            return protocol_to_str_map.at(p);
        }

        static constexpr Protocol protocol_from_str(frozen::string const& name)
        {
            return protocol_from_str_map.at(name);
        }
    };
