...
void nlohmann::adl_serializer<SourceConfig>::from_json(const json& j, iqlogger::config::SourceConfig& sourceConfig)
{
    ...
    try {
        ...
        j.at("overflow_stategy").get_to(sourceConfig.overflow_stategy);
    }
    ...
    for (auto it = j.begin(); it != j.end() ; ++it)
    {
        ...
        if(key != "name" && key != "type" && key != "threads" && key != "processor" && key != "max_queue_size" && key != "overflow_stategy")
        {
...
{
    bool result = std::visit([this](const auto& sourceConfigCheckImpl) -> bool {
        return sourceConfigCheckImpl(*this);
    }, m_checker);
...
#ifdef IQLOGGER_WITH_PROCESSOR
void nlohmann::adl_serializer<ProcessorScript>::from_json(const json& j, iqlogger::config::ProcessorScript& processorScript)
{
    auto script = j.get<std::string>();

    DEBUG("Processor source: " << script);

    auto fullPath = script, configPath = iqlogger::config::ConfigManager::getInstance()->getConfigPath().string() + '/' + script;
...
{
    return processor::Engine::getInstance()->checkScript(source);
}
#endif
