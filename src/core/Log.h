...
{\
  if (logger) {\
    ...
    if constexpr(llevel == frozen::string("warn") || llevel == frozen::string("err") || llevel == frozen::string("critical"))\
...
#define TRACE(...)\
  GET_LOGGER_MACRO(__VA_ARGS__, TRACE_LOGGER, TRACE_DEF_LOGGER)(__VA_ARGS__)
#define DEBUG(...)\
  GET_LOGGER_MACRO(__VA_ARGS__, DEBUG_LOGGER, DEBUG_DEF_LOGGER)(__VA_ARGS__)
#define INFO(...)\
  GET_LOGGER_MACRO(__VA_ARGS__, INFO_LOGGER, INFO_DEF_LOGGER)(__VA_ARGS__)
#define WARNING(...)\
  GET_LOGGER_MACRO(__VA_ARGS__, WARNING_LOGGER, WARNING_DEF_LOGGER)(__VA_ARGS__)
#define ERROR(...)\
  GET_LOGGER_MACRO(__VA_ARGS__, ERROR_LOGGER, ERROR_DEF_LOGGER)(__VA_ARGS__)
#define CRITICAL(...)\
  GET_LOGGER_MACRO(__VA_ARGS__, CRITICAL_LOGGER, CRITICAL_DEF_LOGGER)(__VA_ARGS__)
