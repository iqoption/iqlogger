    ...
    m_bad_chunks{0}
{
    ...
    metrics::MetricsController::getInstance()->registerMetric("inputs." + m_name + ".bad_chunks", [this]() {
        return m_bad_chunks.load();
    });

    m_serverPtr = std::make_shared<Server>(m_inputQueuePtr, m_chunkQueuePtr, m_io_service, m_port);
}

void GelfInput::start()
{
        ...
        while(!isStopped())
        {
                    ...
                    if(messageIt != chunkMap.end())
                    {
                        ...
                        if(messagePtr->chunks.size() == messagePtr->size)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if(!m_inputQueuePtr->enqueue(std::make_unique<Record<Gelf>>(totalMessage.str(), it->second->getSource())))
                                    {
            ...
            for (auto it = chunkMap.cbegin(); it != chunkMap.cend();)
            {
                if (std::chrono::duration_cast<std::chrono::seconds>(now - it->second->time).count() >= m_chunk_ttl)
                {
                    ...
                    WARNING("Chunk Buffer erase... TTL... " << it->second->chunks.size() << "/" << it->second->size << " from " << exampleChunk->second->getSource());
