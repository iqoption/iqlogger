...
TailMonitor::TailMonitor(const std::string& name, RecordQueuePtr<Tail> queuePtr, DelimiterRegex&& startmsg_regex, const std::string& path, bool followOnly, bool saveState) :
    m_name(name),
    ...
    m_saveState(saveState)
{
    ...
    auto notifier = [this](EventPtr eventPtr) {
        ...
        TRACE("Notifier: " << eventPtr->m_watchDescriptor << " EventType: " << eventPtr->m_eventType << " File: " << eventPtr->m_filename);
...
TailMonitor::PointersTableInternalRecord::PointersTableInternalRecord(const std::string& file, DelimiterRegex startmsg_regex, RecordQueuePtr<Tail> queuePtr, bool followOnly, bool saveState) :
        m_fileName(file),
...
{
    return std::make_unique<PointersTableInternalRecord>(filename, m_startMsgRegEx, m_queuePtr, m_followOnly, m_saveState);
}
