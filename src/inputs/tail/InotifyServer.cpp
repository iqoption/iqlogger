...
void InotifyServer::begin_read()
{
    ...
    m_stream.async_read_some(boost::asio::buffer(m_read_buffer), m_strand.wrap(handler));
}
        ...
        while (m_pending_read_buffer.size() >= sizeof(inotify_event))
        {
            ...
            switch (iev->mask)
            {
                ...
                case IN_CREATE | IN_ISDIR:
                    TRACE("Inotify IN_CREATE | IN_ISDIR: " << iev->wd << " Name: " << iev->name << " Mask: " << std::hex << iev->mask);
                    break;
...
void InotifyServer::addWatch(const std::string& path, notifier_t notifier)
{
...

    {
        ...
        wd_accessor->second->addNotifier(notifier, std::regex(pattern));
    }
...
void InotifyServer::start()
{
    TRACE("InotifyServer::start()");

    // @TODO Перенести логику "Started Once" в какой-то класс
...
InotifyServer::Watch::Watch(const std::string& directory) :
    m_directory(directory)
{
    TRACE("InotifyServer::Watch::Watch()");
}

void InotifyServer::Watch::addNotifier(const notifier_t notifier, const std::regex& regex)
{
    TRACE("InotifyServer::Watch::addNotifier()");
    m_notifiers.emplace_back(std::make_unique<InotifyServer::Watch::WatchNotifier>(regex, notifier));
}

void InotifyServer::Watch::notify(EventPtr eventPtr) const
{
    TRACE("InotifyServer::Watch::notify()");
    TRACE("Notify: " << eventPtr->m_watchDescriptor << " Name: " << eventPtr->m_filename << " Type: " << eventPtr->m_eventType);
    ...
    for (auto& notifier : m_notifiers)
    {
        if(std::regex_match(fullName, notifier->m_regex))
        {
            ...
            notifier->m_notifier(std::make_unique<Event>(eventPtr->m_watchDescriptor, eventPtr->m_eventType, std::move(fullName)));
        }
        else
        {
