...
std::string Exporter::exportTelegraf() {
  ...
  try {
    ...
    oss << "cpu_user_seconds_total=" << metrics.at("process.cpu_user_seconds_total")
        ...
        << "i,max_fds=" << metrics.at("process.max_fds") << "i,threads=" << metrics.at("process.threads") << "i "
        << time << std::endl;
    oss_result << oss.str();
  ...
  try {
    ...
    oss << "queue_length=" << metrics.at("processor.queue.length") << "i " << time << std::endl;
    ...
    for (boost::property_tree::ptree::value_type& processor : ptree.get_child("processor")) {
      if (processor.first != "queue") {
        oss << m_measurement << "_processor,processor=" << processor.first
            << " heap_size=" << metrics.at("processor." + processor.first + ".heap_size") << "i " << time << std::endl;
      }
    }
  ...
  try {
    ...
    oss << "queue_length=" << metrics.at("main_queue.length") << "i " << time << std::endl;
    oss_result << oss.str();
  } catch (const std::out_of_range& ex) {
    DEBUG("Exception out of range: " << ex.what());
  }
  ...
  try {
    ...
    for (boost::property_tree::ptree::value_type& input : ptree.get_child("inputs")) {
      if (input.first != "receive_total") {
        ...
        visit_if(
            input.second,
            [&oss, &i](boost::property_tree::ptree::path_type const& path, boost::property_tree::ptree const& node) {
              if (i++)
                oss << ",";
